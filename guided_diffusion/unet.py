...
class FFParser(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        """
        # 如果输入张量 x 的高 H 和宽 W不是偶数，就增加1，确保其成为偶数
        H_pad = H + (H % 2)
        W_pad = W + (W % 2)
        # 创建一个与输入张量 x 相同形状的全0张量 x_pad
        x_pad = torch.zeros((B, C, H_pad, W_pad), dtype=x.dtype, device=x.device)
        # 将输入张量 x 的内容复制到新创建的 x_pad 中，确保其大小匹配
        x_pad[:, :, :H, :W] = x
        """
        ...
        #print("float:",x.shape)
        # 对 x_pad 进行二维离散傅立叶变换（DFT）
        x = torch.fft.rfft2(x, dim=(2, 3), norm='ortho')
        #print("fft:",x.shape)
        x = torch.roll(x,(H//2,W //2),dims=(2,3)) #移频操作,把低频放到中央
        print("roll:",x.shape)
        b, c, h, w = x.shape
        hpf=torch.ones((h,w)) # 高频滤波器
        # 在中心区域（大小为160x160）设置高通过滤器的值为0
        hpf[int(h / 2 - 30):int(h / 2 + 30), int(w / 2 - 30):int(w / 2 + 30)] = 0
        # 将高通过滤器 hpf_resized 的大小扩展到与 x_pad 相同
        hpf_resized = hpf.unsqueeze(0).unsqueeze(0)  # Add batch and channel dimensions
        hpf_resized = hpf_resized.expand(b, 1, h, w)  # Expand to match the dimensions of x
        hpf_resized = hpf_resized.cuda()
        x =x * hpf_resized # 乘以高频滤波器
        
        weight = torch.view_as_complex(self.complex_weight) # 将权重转换为复数形式
        x = x * weight
...
class UNetModel_v1preview(nn.Module):
    ...
    def __init__(
        ...
        if high_way:
            features = 32
            self.hwm = VGG16(self.in_channels - 1, features, 1)
            #self.aff= AFF(self.in_channels)
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        for u in range(num_pool):
            ...
            if not self.convolutional_upsampling:
                ...
                # 如果使用卷积上采样，创建一个转置卷积操作并将其添加到self.tu列表中。
...
self.norm_op, self.norm_op_kwargs, self.dropout_op, self.dropout_op_kwargs,self.nonlin, self.nonlin_kwargs, basic_block=basic_block)
    ...
    def forward(self, x, hs = None):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            x = self.conv_blocks_context[d](x) # 第d编码conv_block的结果
        ...
        x = self.conv_blocks_context[-1](x) # 瓶颈结构的内容
        emb = conv_nd(2, x.size(1), 512, 1).to(device = x.device)(x)

        for u in range(len(self.tu)):
            x = self.tu[u](x)
            x = th.cat((x, skips[-(u + 1)]), dim=1) #跳跃连接
...
class VGG16(nn.Module):
    def __init__(self,input_channels, base_num_features, num_classes):
        ...
        self.conv1_1 = nn.Conv2d(input_features,64,kernel_size=3,padding=1)
        ...
        self.up5_1 = nn.Conv2d(in_channels=512,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.up5_2 = nn.Conv2d(in_channels=512,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.up5_3 = nn.Conv2d(in_channels=512,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.decon1=nn.Conv2d(192, 512, kernel_size=3, padding=1)
        self.up1=nn.ConvTranspose2d(512, 512, kernel_size=2, stride=2)
        self.up4_1 = nn.Conv2d(in_channels=512,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.up4_2 = nn.Conv2d(in_channels=512,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.up4_3 = nn.Conv2d(in_channels=512,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.decon2=nn.Conv2d(704, 256, kernel_size=3, padding=1)
        self.up2=nn.ConvTranspose2d(256, 256, kernel_size=2, stride=2)
        self.up3_1 = nn.Conv2d(in_channels=256,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.up3_2 = nn.Conv2d(in_channels=256,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.up3_3 = nn.Conv2d(in_channels=256,out_channels=64,kernel_size=3,stride=1,padding=1)
        self.decon3=nn.Conv2d(448, 128, kernel_size=3, padding=1)
        self.up3=nn.ConvTranspose2d(128, 128, kernel_size=2, stride=2)
        self.up2_1 = nn.Conv2d(in_channels=128,out_channels=32,kernel_size=3,stride=1,padding=1)
        self.up2_2 = nn.Conv2d(in_channels=128,out_channels=32,kernel_size=3,stride=1,padding=1)
        self.decon4=nn.Conv2d(192, 64, kernel_size=3, padding=1)
        self.up4=nn.ConvTranspose2d(64, 64, kernel_size=2, stride=2)
        self.up1_1 = nn.Conv2d(in_channels=64,out_channels=32,kernel_size=3,stride=1,padding=1)
        self.up1_2 = nn.Conv2d(in_channels=64,out_channels=32,kernel_size=3,stride=1,padding=1)
        self.decon5=nn.Conv2d(128, self.output_features, kernel_size=3, padding=1)
        self.pred=nn.ConvTranspose2d(self.output_features, self.num_classes, kernel_size=2, stride=2)
        
    def forward(self, img):

        
        conv1_1 = F.relu(self.conv1_1(img))
        conv1_2 = F.relu(self.conv1_2(conv1_1))
        pool1 = F.max_pool2d(conv1_2, kernel_size=2, stride=2, padding=0)
        conv2_1 = F.relu(self.conv2_1(pool1))
        conv2_2 = F.relu(self.conv2_2(conv2_1))
        pool2 = F.max_pool2d(conv2_2, kernel_size=2, stride=2, padding=0)
        conv3_1 = F.relu(self.conv3_1(pool2))
        conv3_2 = F.relu(self.conv3_2(conv3_1))
        conv3_3 = F.relu(self.conv3_3(conv3_2))
        pool3 = F.max_pool2d(conv3_3, kernel_size=2, stride=2, padding=0)
        conv4_1 = F.relu(self.conv4_1(pool3))
        conv4_2 = F.relu(self.conv4_2(conv4_1))
        conv4_3 = F.relu(self.conv4_3(conv4_2))
        pool4 = F.max_pool2d(conv4_3, kernel_size=2, stride=2, padding=0)
        conv5_1 = F.relu(self.conv5_1(pool4))
        conv5_2 = F.relu(self.conv5_2(conv5_1))
        conv5_3 = F.relu(self.conv5_3(conv5_2))
        uemb = F.interpolate(conv5_3, size=(14,14), mode='bilinear', align_corners=False)
        
        up5_1 = F.relu(self.up5_1(conv5_1))
        up5_2 = F.relu(self.up5_2(conv5_2))
        up5_3 = F.relu(self.up5_3(conv5_3))
        concat = torch.cat((up5_1,up5_2,up5_3), dim=1)
        concat = F.relu(self.decon1(concat))
        deconv1 =F.relu(self.up1(concat))

        up4_1 = F.relu(self.up4_1(conv4_1))
        up4_2 = F.relu(self.up4_2(conv4_2))
        up4_3 = F.relu(self.up4_3(conv4_3))
        concat = torch.cat((deconv1,up4_1,up4_2,up4_3), dim=1)
        concat = F.relu(self.decon2(concat))
        deconv2 =F.relu(self.up2(concat))

        up3_1 = F.relu(self.up3_1(conv3_1))
        up3_2 = F.relu(self.up3_2(conv3_2))
        up3_3 = F.relu(self.up3_3(conv3_3))
        concat = torch.cat((deconv2,up3_1,up3_2,up3_3), dim=1)
        concat = F.relu(self.decon3(concat))
        deconv3 =F.relu(self.up3(concat))
        
        up2_1 = F.relu(self.up2_1(conv2_1))
        up2_2 = F.relu(self.up2_2(conv2_2))
        concat = torch.cat((deconv3,up2_1,up2_2), dim=1)
        concat = F.relu(self.decon4(concat))
        deconv4 =F.relu(self.up4(concat))
        
        up1_1 = F.relu(self.up1_1(conv1_1))
        up1_2 = F.relu(self.up1_2(conv1_2))
        concat = torch.cat((deconv4,up1_1,up1_2), dim=1)
        concat = F.relu(self.decon5(concat))
        pred= F.relu(self.pred(concat))
        pred = F.interpolate(pred, size=(448,448), mode='bilinear', align_corners=False)
...
class AFF(nn.Module):
    ...
    def __init__(self, channels): #,channels=8, r=2
        ...
        self.local_att = nn.Sequential(
            ...
            nn.Conv2d(512, channels, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(channels, 512, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(512),
        )
