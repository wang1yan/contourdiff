...
class GaussianDiffusion:
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        """
        if noise is None:
            noise = th.randn_like(x_start).to(dist_util.dev())
    ...
    def training_losses(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        """
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            model_output = model_output.to('cuda:0')
            res = res.to('cuda:0')
            cal = cal.to('cuda:0')
